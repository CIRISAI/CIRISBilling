...
down_revision: str | None = "2025_10_08_0002"
...
def upgrade() -> None:
    ...
    op.create_check_constraint(
        "ck_admin_users_ciris_domain", "admin_users", "email LIKE '%@ciris.ai'"
    )
...
def downgrade() -> None:
    ...
    op.add_column(
        "admin_users", sa.Column("password_hash", sa.Text(), nullable=False, server_default="")
    )
    op.add_column(
        "admin_users",
    ...
    op.drop_constraint("ck_admin_users_role", "admin_users", type_="check")
    op.create_check_constraint(
        "ck_admin_users_role", "admin_users", "role IN ('super_admin', 'admin', 'viewer')"
    )
