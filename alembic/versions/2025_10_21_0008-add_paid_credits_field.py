...
down_revision: str | None = "2025_10_16_0007"
...
def upgrade() -> None:
    ...
    op.add_column(
        "accounts", sa.Column("paid_credits", sa.BigInteger(), nullable=False, server_default="0")
    )

    # Add check constraint for non-negative paid_credits
    op.create_check_constraint("ck_paid_credits_non_negative", "accounts", "paid_credits >= 0")
