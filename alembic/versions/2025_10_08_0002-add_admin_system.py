...
down_revision: Union[str, None] = '2025_10_08_0001'
...
def upgrade() -> None:
    ...
    op.create_table('api_keys',
        ...
        sa.Column('permissions', postgresql.ARRAY(sa.String()), nullable=False, server_default=sa.text("ARRAY['billing:read', 'billing:write']")),
        ...
        sa.CheckConstraint("status IN ('active', 'rotating', 'revoked')", name='ck_api_keys_status'),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('key_prefix')
    )
    op.create_index('idx_api_keys_created_by', 'api_keys', ['created_by'])
    op.create_index('idx_api_keys_prefix_active', 'api_keys', ['key_prefix'], unique=False, postgresql_where=sa.text("status = 'active'"))
    ...
    op.create_table('admin_users',
        ...
        sa.CheckConstraint("role IN ('super_admin', 'admin', 'viewer')", name='ck_admin_users_role'),
        sa.CheckConstraint("email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Z|a-z]{2,}$'", name='ck_admin_users_email_format'),
        sa.PrimaryKeyConstraint('id'),
    ...
    op.create_table('provider_configs',
        ...
        sa.CheckConstraint("provider_type IN ('stripe', 'square', 'paypal')", name='ck_provider_configs_type'),
        sa.PrimaryKeyConstraint('id'),
    ...
    op.create_index('idx_admin_audit_logs_created_at', 'admin_audit_logs', ['created_at'], postgresql_using='brin')
    op.create_index('idx_admin_audit_logs_resource', 'admin_audit_logs', ['resource_type', 'resource_id'])
