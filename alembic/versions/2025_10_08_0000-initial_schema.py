...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "oauth_provider", "external_id", "wa_id", "tenant_id", name="uq_account_identity"
        ),
    )
    ...
    op.create_index(
        "idx_accounts_wa_id", "accounts", ["wa_id"], postgresql_where=sa.text("wa_id IS NOT NULL")
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "balance_after = balance_before - amount_minor", name="ck_charge_balance_consistency"
        ),
        sa.UniqueConstraint("account_id", "idempotency_key", name="uq_charge_idempotency"),
        sa.ForeignKeyConstraint(
            ["account_id"], ["accounts.id"], name="fk_charges_account", ondelete="RESTRICT"
        ),
    )
    ...
    op.create_index(
        ...
        "charges",
        ["metadata_request_id"],
        postgresql_where=sa.text("metadata_request_id IS NOT NULL"),
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "balance_after = balance_before + amount_minor", name="ck_credit_balance_consistency"
        ),
        sa.CheckConstraint(
            "transaction_type IN ('purchase', 'grant', 'refund', 'adjustment')",
            name="ck_transaction_type",
        ),
        sa.UniqueConstraint("account_id", "idempotency_key", name="uq_credit_idempotency"),
        sa.ForeignKeyConstraint(
            ["account_id"], ["accounts.id"], name="fk_credits_account", ondelete="RESTRICT"
        ),
    )
    ...
    op.create_table(
        ...
        # Foreign key
        sa.ForeignKeyConstraint(
            ["account_id"], ["accounts.id"], name="fk_credit_checks_account", ondelete="SET NULL"
        ),
    )
    ...
    op.create_index(
        "idx_credit_checks_oauth_external", "credit_checks", ["oauth_provider", "external_id"]
    )
